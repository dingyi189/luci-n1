...
mtproto_password = s:option(Value, "mtproto_password", translate("Password"))
...
uuid = s:option(DynamicList, "uuid", translate("UUID"))
for i = 1, 3 do
...
tls_certificateFile = s:option(Value, "tls_certificateFile", translate("Public key absolute path"))
...
tls_keyFile = s:option(Value, "tls_keyFile", translate("Private key absolute path"))
...
mkcp_guise = s:option(ListValue, "mkcp_guise", translate("Camouflage Type"))
...
ds_path = s:option(Value, "ds_path", translate("Path"))
...
quic_key = s:option(Value, "quic_key", translate("Encrypt Password"))
...
bind_local = s:option(Flag, "bind_local", translate("Bind Local"))
bind_local.default = "0"

accept_lan = s:option(Flag, "accept_lan", translate("Accept LAN Access"))
